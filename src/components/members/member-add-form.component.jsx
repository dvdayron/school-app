...
import {addMembersStartAsync, fetchMembersStartAsync} from '../../redux/members/members.actions';
 ...
 */
const MemberAddForm = ({type, getMembers, addMember, members, isFetching, cities, getCities, groups, getGroups}) => {
  ...
  const handleSubmit = (e) => {
    ...
    if (name
      ...
      && gender
      && cityId) {
      const unique = members.find((m) => m.email === email);
      if (unique === undefined) {
        addMember({...member, type});
        setSuccess('Miembro adicionado.');
        // change model status
        setmember({
          name: '', 
          email: '',
  ...
  return (
    <form 
      ...
      {
        type !== 'profesores' && groups && groups.length
        ? <FormSelect 
            handleChange={handleChange}
            name="groupId"
            label="Grupo"
...
const mapDispatchToProps = dispatch => ({
  addMember: (member) => dispatch(addMembersStartAsync(member)),
